...
def start_the_game():
    ...
    font = pygame.font.Font('SofiaSans[wdth,wght].ttf',32)
    ...
    def pause():
        PAUSE = pygame_menu.Menu('This round is over. Do you want to continue playing?', WIDTH / 2, HEIGHT / 2, theme=pygame_menu.themes.THEME_DARK)
        PAUSE.add.button('Continue', start_the_game)
        PAUSE.add.button('Quit', pygame_menu.events.QUIT)


    pygame.time.set_timer(pygame.NUMEVENTS,60000)

    if pygame.time.set_timer == 0:
        pause()
